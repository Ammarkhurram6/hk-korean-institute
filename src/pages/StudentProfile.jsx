...
function StudentProfile() {
  ...
  const populateDrafts = (s) => {
    ...
    setCustomCourse(
      s.course ? !Object.keys(COURSE_FEES).includes(s.course) : false,
    );
  };
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        "success",
        editingPaymentId ? "Payment updated." : "Payment added.",
      );
  ...
  const handleDeletePayment = async (p) => {
    if (
      !window.confirm(
        `Delete payment of ${formatPKR(p.amount)}? This cannot be undone.`,
      )
    )
      return;
    try {
      const res = await fetch(
        `${API_URL}/api/admin/students/${id}/payments/${p._id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        },
    ...
    <div className="min-h-screen bg-lightgray dark:bg-charcoal transition-colors">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        >
          ...
          <div className="flex-grow text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-2 justify-center md:justify-start mt-3">
              {student.course && (
                <span className="px-3 py-1 bg-kred/10 text-kred rounded-full text-xs font-bold">
                  {student.course}
                </span>
              )}
              <span
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-bold ${feeStatus === "Paid" ? "bg-green-100 text-green-700" : feeStatus === "Partially Paid" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}
              >
                {feeStatus}
              </span>
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-8">
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-xl bg-navy/10 dark:bg-white/10 text-navy dark:text-white flex items-center justify-center">
                <FiUser />
              </div>
              ...
              {[
                ...
                [
                  ...
                  student.identityNumber
                    ? `${student.identityType || ""}: ${student.identityNumber}`
                    : "—",
                ],
                ["Email", student.email || "—"],
                ["Phone", student.phone || "—"],
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  value={customCourse ? "__custom__" : courseForm.course}
                  onChange={(e) => {
                    ...
                    setCourseForm((p) => ({ ...p, course: val }));
                    // Course ke hisaab se fee auto-fill
                    const fee = getCourseFee(val);
                    if (fee > 0) setFeeForm({ totalFee: fee });
                  }}
                  className={inputClass}
                >
                ...
                {/* Custom course ka text input — sirf "Other" select par dikhega */}
                {customCourse && (
                  <input
                    type="text"
                    value={courseForm.course}
                    onChange={(e) =>
                      setCourseForm((p) => ({ ...p, course: e.target.value }))
                    }
                ...
                {/* Selected course ki fee hint */}
                {getCourseFee(courseForm.course) > 0 && (
                  <p className="text-xs text-green-600 font-medium mt-1.5">
                    ✓ Fee : {formatPKR(getCourseFee(courseForm.course))}
                  </p>
                )}
              </div>
              <div>
              ...
              {courseForm.courseDuration === "Custom" && (
                <div>
                  <label className={labelClass}>
                    Custom Duration (e.g. 75 Days)
                  </label>
                  <input
                    type="text"
                    value={courseForm.customDuration}
                    onChange={(e) =>
                      setCourseForm((p) => ({
                        ...p,
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={courseForm.lastDay}
                    onChange={(e) =>
                      setCourseForm((p) => ({ ...p, lastDay: e.target.value }))
                    }
                    className={inputClass}
                  />
                </div>
              ...
              <p className="text-xs text-gray-400">
                Last Day auto-calculates from Duration + Joining Date, but you
                can change it manually.
              </p>
              <button
                onClick={() =>
                  updateStudent(
                    { ...courseForm, totalFee: Number(feeForm.totalFee) },
                    "course",
                    "Course information saved.",
                  )
                }
        ...
        >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-green-50 dark:bg-green-500/10 rounded-2xl p-4">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {student.payments?.length || 0} payment(s)
              </p>
            </div>
            ...
            <div className="bg-blue-50 dark:bg-blue-500/10 rounded-2xl p-4 flex flex-col justify-center">
              ...
              <span
                className={`inline-block px-3 py-1.5 rounded-full text-xs font-bold w-fit ${feeStatus === "Paid" ? "bg-green-100 text-green-700" : feeStatus === "Partially Paid" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}
              >
                {feeStatus}
              </span>
            </div>
          ...
          {/* Payment History */}
          <h4 className="text-sm font-bold text-navy dark:text-white uppercase tracking-wider mb-3">
            Payment History
          </h4>
          ...
          <form
            ...
            className="bg-gray-50 dark:bg-white/5 rounded-2xl p-5"
          >
            ...
            <div className="grid sm:grid-cols-4 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPaymentForm((p) => ({ ...p, details: e.target.value }))
                  }
                  className={inputClass}
            ...
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBookForm((p) => ({ ...p, bookName: e.target.value }))
                  }
                  className={inputClass}
                  placeholder="EPS TOPIK Korean Book"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  type="date"
                  value={bookForm.orderDate}
                  onChange={(e) =>
                    setBookForm((p) => ({ ...p, orderDate: e.target.value }))
                  }
                  className={inputClass}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBookForm((p) => ({ ...p, purchaseDate: e.target.value }))
                  }
                  className={inputClass}
                />
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setBookForm((p) => ({ ...p, details: e.target.value }))
                  }
                  className={`${inputClass} resize-none`}
                  placeholder="Student purchased the complete EPS TOPIK preparation book set."
                />
              </div>
            </motion.div>
          )}
              ...
              updateStudent(
                {
                  book: {
                    ...
                    orderDate: bookForm.orderDate || null,
                    purchaseDate: bookForm.purchaseDate || null,
                  },
                },
                "book",
        ...
        >
          ...
          <button
            onClick={() =>
              updateStudent({ notes: notesForm }, "notes", "Notes saved.")
            }
